...
void main() {
    ...
    shadow = mix(1.0, mix(0.55, 1.0, shadow), dayFactor);
#else
    shadow = 1.0;
#endif

    float upDot   = clamp(vNormal.y, 0.0, 1.0);
    float aoTerm  = mix(0.80, 1.0, upDot);
    ...
    vec3 ambient  = (ambSky * (0.45 + 0.45 * skylight) + ambFloor) * aoTerm;
    ...
    fogCol = mix(fogCol, vec3(0.55, 0.58, 0.62), rainStrength * 0.6);
    lit = mix(lit, fogCol, fogF);

    vec3 nEnc = encodeNormal(vNormal);

    gl_FragData[0] = vec4(lit, 1.0);
    gl_FragData[1] = vec4(nEnc, RT_ROUGHNESS_TERRAIN);
}
#endif
