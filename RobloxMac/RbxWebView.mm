...
- (void)windowDidLoad
{
    ...
    self.webView.customUserAgent = [NSString stringWithFormat:@"Mozilla/5.0 (Macintosh; Intel Mac OS X %@) AppleWebKit/536.25 (KHTML, like Gecko) %s",versionString, FString::ClientExternalBrowserUserAgent.c_str()];
}
...
- (void)windowWillClose:(NSNotification *)notification
{
    ...
    if (closingWindow && closingWindow == self.window )
    {
        ...
        // WKWebView does not have resourceLoadDelegate
        [self autorelease];
    }
}
