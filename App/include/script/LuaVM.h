...
#include <string>
#if defined(RBX_SECURE_DOUBLE) && (defined(__x86_64__) || defined(__i386__))
#include <emmintrin.h>
#endif
...
public:
...
#else
        uint64_t key, bits;
        std::memcpy(&key, luaXorMask, sizeof(uint64_t));
        std::memcpy(&bits, &storage, sizeof(uint64_t));
        bits ^= key;
        double result;
    ...
    void operator=(const double& value)
    {
        ...
        bits ^= key;
        std::memcpy(&storage, &bits, sizeof(double));
#endif
    }
